...
func checkAndRenameTables(jobCtx *jobContext, t *meta.Mutator, job *model.Job, tblInfo *model.TableInfo, args *model.RenameTableArgs) (ver int64, _ error) {
	...
	err = updateLabelRules(jobCtx.store.GetCodec(), job, tblInfo, oldRules, tableRuleID, partRuleIDs, oldRuleIDs, tblInfo.ID)
	if err != nil {
		job.State = model.JobStateCancelled
		return ver, errors.Wrapf(err, "failed to update the label rule to PD")
	}
...
func getOldLabelRules(codec tikv.Codec, tblInfo *model.TableInfo, oldSchemaName, oldTableName string) (string, []string, []string, map[string]*label.Rule, error) {
	...
	if tblInfo.GetPartitionInfo() != nil {
		for _, def := range tblInfo.GetPartitionInfo().Definitions {
			id := label.NewRuleID(codec, oldSchemaName, oldTableName, def.Name.L)
			partRuleIDs = append(partRuleIDs, id)
		}
	}
