...
func (w *GCWorker) leaderTick(ctx context.Context) error {
	...
	*/
	if keyspace.IsKeyspaceMetaNotNilAndUseGlobalGC(w.store.GetCodec().GetKeyspaceMeta()) {
		failpoint.Inject("testKsInGlobalGC", func(v failpoint.Value) {
			ms := v.(int)
			time.Sleep(time.Duration(ms) * time.Millisecond)
			failpoint.Return(errors.New("testKsInGlobalGCInjectedError"))
		})
...
func (w *GCWorker) resolveLocks(
	...
	err := w.resolveLocksWithBounded(ctx, runner, safePoint)
	if err != nil {
...
func (w *GCWorker) resolveLocksWithBounded(ctx context.Context, runner *rangetask.Runner, safePoint uint64) error {
	var err error
	var txnLeftBound []byte
	var txnRightBound []byte

	if w.isCurrentKeyspaceUseKeyspaceLevelGC() {
		failpoint.Inject("testLegacyResolveKeyspaceLocks", func(v failpoint.Value) {
			ms := v.(int)
			time.Sleep(time.Duration(ms) * time.Millisecond)
			failpoint.Return(errors.New("ResolveKeyspaceLocksInjectedError"))
		})
