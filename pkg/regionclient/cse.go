...
var (
	// StoresRefreshInterval is the interval to refresh stores from PD.
	StoresRefreshInterval = time.Second * 5
	// SyncRegionTimeout is the timeout for each sync region request.
	SyncRegionTimeout = time.Second * 2
)

var (
	// ErrRegionNotFound indicates that the region is not found.
...
// Client is a pd client which fetches region info from TiKV stores directly.
type Client struct {
	pd.Client
...
// SyncRegionRequest is the request body for sync region request for CSE.
...
// SyncRegionByIDRequest is the request body for sync region by id request for CSE.
...
// GetRegion implements pd.Client interface
...
// GetPrevRegion implements pd.Client interface
...
// GetRegionByID implements pd.Client interface
...
// ScanRegions implements pd.Client interface
...
// GetStore implements pd.Client interface
