...
func TestFTSConfigDisable(t *testing.T) {
	...
	tk.MustContainErrMsg(`create table t(
		...
		FULLTEXT KEY (title)
	)`, "Unsupported FULLTEXT index")

	tk.MustExec(`create table t(id INT, title TEXT, body TEXT)`)
	tk.MustExec("insert into t values (1, 'hello world', 'hello world')")
	tk.MustContainErrMsg(`alter table t add FULLTEXT INDEX (title)`, "Unsupported FULLTEXT index")
	tk.MustContainErrMsg(`create FULLTEXT INDEX idx on t(title)`, "Unsupported FULLTEXT index")
}
...
func TestFTSIndexSyntax(t *testing.T) {
	...
	tk.MustContainErrMsg("alter table t1 add FULLTEXT INDEX (body)", "columnar replica must exist to create fulltext index")
