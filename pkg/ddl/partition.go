...
func dropLabelRules(jobCtx *jobContext, schemaName, tableName string, partNames []string) error {
	deleteRules := make([]string, 0, len(partNames))
	for _, partName := range partNames {
		id := label.NewRuleID(jobCtx.store.GetCodec(), schemaName, tableName, partName)
		deleteRules = append(deleteRules, id)
	}
	// delete batch rules
	patch := label.NewRulePatch([]*label.Rule{}, deleteRules)
	return infosync.UpdateLabelRules(jobCtx.ctx, patch)
}
...
func (w *worker) rollbackLikeDropPartition(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	err = dropLabelRules(jobCtx, job.SchemaName, tblInfo.Name.L, pNames)
	if err != nil {
		job.State = model.JobStateCancelled
...
func (w *worker) onDropTablePartition(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	case model.StateWriteOnly:
		...
		updateDroppingPartitionInfo(tblInfo, partNames)
		err = dropLabelRules(jobCtx, job.SchemaName, tblInfo.Name.L, partNames)
		if err != nil {
...
func replaceTruncatePartitions(jobCtx *jobContext, job *model.Job, t *meta.Mutator, tblInfo *model.TableInfo, oldIDs, newIDs []int64) ([]model.PartitionDefinition, []model.PartitionDefinition, error) {
...
func (w *worker) onExchangeTablePartition(jobCtx *jobContext, job *model.Job) (ver int64, _ error) {
	...
	ntrID := label.NewRuleID(jobCtx.store.GetCodec(), job.SchemaName, nt.Name.L, "")
	ptrID := label.NewRuleID(jobCtx.store.GetCodec(), job.SchemaName, pt.Name.L, partDef.Name.L)
